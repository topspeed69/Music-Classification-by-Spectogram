...
Convert audio files to mel-spectrograms with multiprocessing support
"""
...
LOG_FILE = Path(__file__).parent / 'failed_audio_files_mel.log'
...
def audio_to_mel_spectrogram(audio_path, save_path, duration, n_mels=128):
    """
    Convert an audio file to a mel-spectrogram and save it as an image.

    Args:
        audio_path (str): The path to the audio file.
        save_path (str): The path to save the mel-spectrogram image.
    ...
    S_dB = cv2.normalize(S_dB, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)

    # Convert to RGB and save as PNG
    S_dB = cv2.cvtColor(S_dB, cv2.COLOR_GRAY2RGB)
    cv2.imwrite(save_path, S_dB)
    return True
...
def load_audio_safe(path, duration=None, sr=22050):
    ...
    On irrecoverable failure, append an entry to `failed_audio_files_mel.log` and return (None, None).
    """
    try:
...
def process_file(args):
    ...
    file, source, output, duration, n_mels = args
    try:
        # Output path
        ...
        # Convert
        success = audio_to_mel_spectrogram(str(file), str(new_path), duration, n_mels)
        return success
    except Exception:
        return False
...
if __name__ == "__main__":
    ...
    parser.add_argument('--source', type=str, default='dataset', help='source folder')
    parser.add_argument('--duration', type=int, default=60, help='duration of audios in case they are too big')
    parser.add_argument('--output', type=str, default='output_mel', help='folder output')
    parser.add_argument('--workers', type=int, default=5, help='number of parallel workers (default: CPU count)')
    ...
    print(f"Generating mel-spectrograms with {n_mels} mel bands")

    # Prepare arguments for multiprocessing
    args_list = [(file, source, output, duration, n_mels) for file in file_list]
    ...
    print(f"\n{'='*60}")
    print(f"Mel-spectrogram processing complete!")
